...
function Daftar() {
  ...
  const getKelasJurusan = async () => {
    try {
      ...
      setKelasList(["Semua", ...new Set(res.data.map(k=>k.kelas).filter(Boolean))]);
      setJurusanList(["Semua", ...new Set(res.data.map(k=>k.jurusan).filter(Boolean))]);
    } catch (err) { console.error(err); }
  };
  ...
  const filteredData = data.filter(d=>{
    ...
    return matchKategori && matchKelas && matchJurusan && matchSearch && isKategoriAktif;
  });
      ...
      <div className="min-h-screen p-8 flex justify-center bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="w-full max-w-6xl space-y-8">
          ...
          <div className="flex flex-wrap gap-3 mb-6 items-center">
            <input placeholder="Cari nama..." value={search} onChange={e=>setSearch(e.target.value)} className="border p-2 rounded"/>
            <select value={filterKategori} onChange={e=>setFilterKategori(e.target.value)} className="border p-2 rounded">
            ...
            <select value={filterJurusan} onChange={e=>setFilterJurusan(e.target.value)} className="border p-2 rounded">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-2xl border border-gray-200 overflow-x-auto">
            <div className="flex justify-between mb-4">
              <h3 className="text-xl font-bold">Daftar Guru, Siswa, dan Karyawan</h3>
                ...
                {filteredData.length>0 ? filteredData.map((item,index)=>(
                  <tr key={item.id} className={`${index%2===0?"bg-gray-50":"bg-gray-100"} hover:bg-blue-50`}>
                    ...
                    <td className="p-3 flex gap-2 justify-center">
                      <button onClick={()=>navigate(`/EditData/${item.id}`)} className="bg-blue-500 text-white px-3 py-1 rounded">Edit</button>
                      <button onClick={()=>handleDelete(item.id)} className="bg-red-500 text-white px-3 py-1 rounded">Hapus</button>
                    </td>
                  </tr>
                )) : (
                  <tr><td colSpan="8" className="p-4 text-center text-gray-500 italic">Tidak ada data untuk filter ini</td></tr>
                )}
              </tbody>
            </table>
          </div>
